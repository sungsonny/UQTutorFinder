// Copyright 2017-2018 Leaning Technologies Ltd. All Rights Reserved.
...
function CheerpJFolder(mp)
{
	...
	// { statAsync, listAsync, makeFileData, createDirAsync, loadAsync, renameAsync }
	this.mountOps = null;
...
var RootOps = { statAsync: rootStatAsync, listAsync: rootListAsync, makeFileData: null, createDirAsync: null, loadAsync: null, renameAsync: null };
	...
	xhr.onload = function(e) {
		...
		if(xhr.status == 200)
		{
			...
			else if(hdr = xhr.getResponseHeader("Content-Length"))
			{
				...
				cheerpjDownload = new Uint8Array(xhr.response);
			}
...
var WebOps = { statAsync: webStatAsync, listAsync: webListAsync, makeFileData: webMakeFileData, createDirAsync: null, loadAsync: webLoadAsync, renameAsync: null };
...
function idbResolve(mp, tx, store, path, fileRef, p)
{
	var parentPath = null;
	// We get here after dbConnection is valid
	if(path.length == 0)
	...
	function gotParent(res, asyncThread)
	{
		...
		if(filePath == "")
		{
			gotFile(parentRes, parentRes, asyncThread !== null);
			return false;
		}
		else if(fileIndex < 0)
		{
			gotFile(parentRes, null, asyncThread !== null);
			return false;
		}
		else
		{
...
function idbMakeFileData(mp, path, mode, p)
{
	...
	var tx = a.tx = mp.dbConnection.transaction("files", "readwrite");
	var store = a.store = tx.objectStore("files");
	a.pc=1;idbResolve(mp, tx, store, path, fileRef, a);
	var doAsync = false;
...
var IdbOps = { statAsync: idbStatAsync, listAsync: idbListAsync, makeFileData: idbMakeFileData, createDirAsync: idbCreateDirAsync, loadAsync: null, renameAsync: idbRenameAsync };
...
var StrOps = { statAsync: strStatAsync, listAsync: null, makeFileData: null, createDirAsync: null, loadAsync: strLoadAsync, renameAsync: null };
...
var DevOps = { statAsync: devStatAsync, listAsync: null, makeFileData: devMakeFileData, createDirAsync: null, loadAsync: null, renameAsync: null };
	...
	else
	{
		...
		if(mount.hasReadWrite)
		{
			...
			if(fileRef.permType==0)
				return -1;
			fileData = new CheerpJFileData(mount, path.substr(mount.mountPoint.length-1), fileRef.cheerpjDownload ? fileRef.cheerpjDownload.length|0 : 0, fileRef.inodeId|0, fileRef.permType|0);
